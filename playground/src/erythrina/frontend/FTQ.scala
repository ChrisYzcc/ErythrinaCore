...
class FTQ extends ErythModule {
    val io = IO(new Bundle {
        ...
        val pred_rsp = ValidIO(new InstFetchBlock)      // rsp from BPU
    ...
    for (i <- 0 until FTQSize) {
        val entry = entries(i)
        when (predicted(i) && fetched(i) && deqPtr === i.U) {
            decode_req.bits := entry
            decode_req.valid := true.B
        }
    }
    ...
    pred_req.valid := fetch_resp.fire
